                                ...
                                C_Timer.NewTicker(1, function(self)
                                    ...
                                    if x == 0 then
                                        ...
                                        if x == cTime then
                                            ...
                                            if 1 == 1 or dbmBigwigs == 3 then
                                                C_PartyInfo.DoCountdown(cTime)
                                            end
                                            if 2 == 2 or dbmBigwigs == 3 then
...
local cbCancelOnPullTimer = addon.functions.createCheckbox("cancelPullTimerOnClick", frame,
    L["Cancel Pull Timer on click"], 10, (addon.functions.getHeightOffset(cbCloseBags)))

local cbCancelOnPullTimer = addon.functions.createCheckbox("noChatOnPullTimer", frame, L["noChatOnPullTimer"], 10,
    (addon.functions.getHeightOffset(cbCancelOnPullTimer)))
...
local longSlider = addon.functions.createSlider("pullTimerLongTime", frame, L["sliderLongTime"], 15,
    (addon.functions.getHeightOffset(cbAutoKeyStart) - 20), addon.db["pullTimerLongTime"], 1, 60, "s")

local shortSlider = addon.functions.createSlider("pullTimerShortTime", frame, L["sliderShortTime"], 15,
    (addon.functions.getHeightOffset(longSlider) - 20), addon.db["pullTimerShortTime"], 1, 60, "s")
