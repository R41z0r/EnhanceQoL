...
L["vendorAddItemToInclude"] = "Добавьте предметы в этот список, чтобы включить их в продажу.\nВАЖНО: Это игнорирует все остальные проверки для этого предмета"
L["vendorAddItemToExclude"] = "Добавьте предметы в этот список, чтобы исключить их из продажи.\nВАЖНО: Исключение всегда побеждает, если только это не хлам"
...
for _, key in ipairs(addon.Vendor.variables.tabKeyNames) do
    ...
    L["vendor" .. value .. "Enable"] =
        "Включить автоматическую продажу предметов\nкачества " ..
            ITEM_QUALITY_COLORS[key].hex .. _G["ITEM_QUALITY" .. key .. "_DESC"] .. "|r"
    L["labelExplained" .. value .. "line"] =
        "Это означает, что автоматически продаются " .. ITEM_QUALITY_COLORS[key].hex ..
            _G["ITEM_QUALITY" .. key .. "_DESC"] ..
            "|r\nпредметы с уровнем предмета %s и ниже\n%s"
end
