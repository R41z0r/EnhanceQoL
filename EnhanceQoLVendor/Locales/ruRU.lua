...
L["vendorOnly12Items"] =
	"Ограничивает количество продаваемых предметов за одну транзакцию до 12, что упрощает выкуп у торговца."
...
L["vendorAddItemToInclude"] =
	"Добавьте предметы в этот список, чтобы включить их в продажу. ВАЖНО: Это игнорирует все остальные проверки для этого предмета"
L["vendorAddItemToExclude"] =
	"Добавьте предметы в этот список, чтобы исключить их из продажи. ВАЖНО: Исключение всегда побеждает, если только это не хлам"
L["vendorMinIlvl"] = "Отмечать предметы с уровнем ниже выбранного для автопродажи"
L["vendorAbsolutIlvl"] =
	"Использовать абсолютный уровень предмета для продажи, вместо минимального экипированного уровня"
