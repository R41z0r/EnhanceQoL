...
function addon.Aura.functions.addBuffTrackerOptions(container)
        ...
        local sizeSlider = addon.functions.createSliderAce(L["buffTrackerIconSizeHeadline"] .. ": " .. addon.db["buffTrackerSize"], addon.db["buffTrackerSize"], 20, 80, 1, function(self, _, val)
	...
	local edit
        ...
        local buffData = {}
        for id, name in pairs(addon.db["buffTrackerList"]) do
                table.insert(buffData, { id = id, name = name })
        end
